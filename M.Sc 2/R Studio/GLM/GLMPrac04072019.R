...
d1<-iris[1:100,]
head(d1)
IRLS<-function(X,Y,max.iter){
  n<-length(as.vector(Y))
  py<-as.vector(table(Y)/sum(table(Y)))
  y<-vector("numeric")
  for (i in 1:n) {
    y[i]<-ifelse(Y[i]==0, py[1], py[2])
  }
  X1<-as.matrix(cbind.data.frame(ones=rep(1, times=dim(X)[1]),X))
  b<-as.vector(rep(0, times = dim(X1)[2]))
  for (i in 1:max.iter) {
    eta<-X1%*%b
    mu<-exp(eta)/(1+exp(eta))
    nu<-mu*(1-mu)
    w<-n*nu
    z<-eta+((y-mu)/nu)
    bnew<-as.vector(na.omit(lm(z~X1, weights = w)$coefficients))
    b<-bnew  
  }
  return(list("Coef"=b, "mu"=mu, "nu"=nu, "w"=w))
}
levels(Y)<-c(0,1)
d1$Species<-as.factor(as.numeric(d1$Species))
levels(d1$Species)<-c(0,1)
IRLS(X,Y,1000)
